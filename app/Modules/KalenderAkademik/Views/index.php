...
<div class='container-xl'>
    <div class='card shadow-sm border-0'>
        <div class='card-header'>
            <h3 class='card-title'>DATA KALENDERAKADEMIK</h3>
        ...
        <div class='table-responsive p-3'>
            <table id='t-kalender-akademik' class='table table-vcenter card-table table-striped w-100'>
                <thead>
                    <tr>
                        <th width='5%'>NO</th>
                            <th>TAHUN AJARAN</th>
...
<script>
    ...
    $(document).ready(function() {
        table = $('#t-kalender-akademik').DataTable({
            processing: true, 
            serverSide: true,
            order: [[1, 'asc']],
            ajax: { 
                url: '<?= base_url('kalender-akademik/ajaxData') ?>', 
                type: 'POST', 
                data: d => { d.<?= csrf_token() ?> = '<?= csrf_hash() ?>'; } 
            },
            columns: [
                { data: 'no', orderable: false, searchable: false, className: 'text-center' },
                { data: 'tahun_ajaran_id', defaultContent: '-', className: 'text-nowrap' },
                { data: 'semester_id', defaultContent: '-', className: 'text-nowrap' },
                { data: 'kegiatan', defaultContent: '-', className: 'text-nowrap' },
                { data: 'tgl_mulai', className: 'text-nowrap', render: d => Format.tglLong(d) },
                { data: 'tgl_selesai', className: 'text-nowrap', render: d => Format.tglLong(d) },
                { data: 'warna_bg', defaultContent: '-', className: 'text-nowrap' },
    ...
    function addData() { $.get('<?= base_url('kalender-akademik/create') ?>', res => { $('#modal-content-area').html(res); $('#modal-form').modal('show'); }); }
    function editData(id) { $.get('<?= base_url('kalender-akademik/edit/') ?>' + id, res => { $('#modal-content-area').html(res); $('#modal-form').modal('show'); }); }
    function deleteData(id) { deleteId = id; $('#modal-confirm').modal('show'); }

    $('#btn-confirm-del').on('click', function() { 
        $.post('<?= base_url('kalender-akademik/delete/') ?>' + deleteId, { '<?= csrf_token() ?>': '<?= csrf_hash() ?>' }, res => {
