...
class KalenderAkademik extends BaseController
{
    ...
    public function ajaxData() {
        ...
        $builder->select("tbx_kalender_akademik.id, tbx_kalender_akademik.tahun_ajaran_id, tbx_kalender_akademik.semester_id, tbx_kalender_akademik.kegiatan, tbx_kalender_akademik.tgl_mulai, tbx_kalender_akademik.tgl_selesai, tbx_kalender_akademik.warna_bg");
        ...
        if (!empty($files)) {
            foreach ($files as $key => $file) {
                if ($file->isValid() && !$file->hasMoved()) {
                    ...
                    if (isset($data['nisn'])) {
                        $sub = 'siswa/' . $data['nisn'];
                    } elseif (strpos('tbx_kalender_akademik', 'wb_') !== false) {
                        $sub = str_replace('wb_', '', 'tbx_kalender_akademik');
                    }
