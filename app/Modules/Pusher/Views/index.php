...
<div class='container-xl'>
    <div class='card shadow-sm border-0'>
        <div class='card-header'>
            <h3 class='card-title'>DATA PUSHER</h3>
        ...
        <div class='table-responsive p-3'>
            <table id='t-pusher' class='table table-vcenter card-table table-striped w-100'>
                <thead>
                    <tr>
                        <th width='5%'>NO</th>
                            <th>GROUPS</th>
...
<script>
    ...
    $(document).ready(function() {
        table = $('#t-pusher').DataTable({
            processing: true, 
            serverSide: true,
            order: [[1, 'asc']],
            ajax: { 
                url: '<?= base_url('pusher/ajaxData') ?>', 
                type: 'POST', 
                data: d => { d.<?= csrf_token() ?> = '<?= csrf_hash() ?>'; } 
            },
            columns: [
                { data: 'no', orderable: false, searchable: false, className: 'text-center' },
                { data: 'groups', defaultContent: '-', className: 'text-nowrap' },
                { data: 'v_key', defaultContent: '-', className: 'text-nowrap' },
                { data: 'v_value', defaultContent: '-', className: 'text-nowrap' },
                { data: 'description', defaultContent: '-', className: 'text-nowrap' },
    ...
    function addData() { $.get('<?= base_url('pusher/create') ?>', res => { $('#modal-content-area').html(res); $('#modal-form').modal('show'); }); }
    function editData(id) { $.get('<?= base_url('pusher/edit/') ?>' + id, res => { $('#modal-content-area').html(res); $('#modal-form').modal('show'); }); }
    function deleteData(id) { deleteId = id; $('#modal-confirm').modal('show'); }

    $('#btn-confirm-del').on('click', function() { 
        $.post('<?= base_url('pusher/delete/') ?>' + deleteId, { '<?= csrf_token() ?>': '<?= csrf_hash() ?>' }, res => {
