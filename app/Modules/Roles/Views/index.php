...
<div class='container-xl'>
    <div class='card shadow-sm border-0'>
        ...
        <div class='table-responsive p-3'>
            <table id='t-roles' class='table table-vcenter card-table table-striped w-100'>
                <thead>
                    <tr>
                        <th width='5%'>NO</th>
                            <th>NAMA KELOMPOK</th>
...
<div class='modal modal-blur fade' id='modal-form'>
...
<script>
    ...
    $(document).ready(function() {
        table = $('#t-roles').DataTable({
            processing: true, 
            serverSide: true,
            order: [[1, 'asc']],
            ajax: { 
                url: '<?= base_url('roles/ajaxData') ?>', 
                type: 'POST', 
                data: d => { d.<?= csrf_token() ?> = '<?= csrf_hash() ?>'; } 
            },
            columns: [
                { data: 'no', orderable: false, searchable: false, className: 'text-center' },
                { data: 'role_name', defaultContent: '-', className: 'text-nowrap' },
    ...
    function addData() { $.get('<?= base_url('roles/create') ?>', res => { $('#modal-content-area').html(res); $('#modal-form').modal('show'); }); }
    function editData(id) { $.get('<?= base_url('roles/edit/') ?>' + id, res => { $('#modal-content-area').html(res); $('#modal-form').modal('show'); }); }
    function deleteData(id) { deleteId = id; $('#modal-confirm').modal('show'); }

    $('#btn-confirm-del').on('click', function() { 
        $.post('<?= base_url('roles/delete/') ?>' + deleteId, { '<?= csrf_token() ?>': '<?= csrf_hash() ?>' }, res => {
    ...
    $.get('<?= base_url('roles/manage_access') ?>/' + id, function(res) {
