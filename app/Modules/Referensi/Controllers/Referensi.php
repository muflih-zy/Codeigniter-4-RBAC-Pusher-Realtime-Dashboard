...
use Modules\Referensi\Models\ReferensiModel;
...
{
    protected $model;
    protected $moduleUrl = 'referensi';

    public function __construct() {
        $this->model = new ReferensiModel();
    }
    ...
    public function index() {
        ...
        return view('Modules\Referensi\Views\index', [
    ...
    public function ajaxData() {
        ...
        $builder = $db->table('auth_referensi');
        $builder->select("auth_referensi.id, auth_referensi.RfGroup, auth_referensi.Rfid, auth_referensi.RfName, auth_referensi.notes");
        $builder->where('auth_referensi.is_deleted','N');
    ...
    public function create() {
        ...
        return view('Modules\Referensi\Views\form', ['url' => $this->moduleUrl]); 
    }

    public function edit($id) {
        if ($this->getAccess()['can_update'] != 1) return 'Akses dilarang';
        $data = $this->model->find($id);
        return view('Modules\Referensi\Views\form', ['row' => $data, 'url' => $this->moduleUrl]);
    }
    ...
    public function store() {
        ...
        if ($this->model->save($data)) {
            $this->notify('auth_referensi');
            ActivityLogModel::saveLog('INSERT', "Menambah Referensi", 'ti ti-plus', 'success');
    ...
    public function update($id) {
        ...
        if ($this->model->update($id, $data)) {
            $this->notify('auth_referensi');
            ActivityLogModel::saveLog('UPDATE', "Update Referensi ID: $id", 'ti ti-edit', 'info');
        ...
        if (!empty($files)) {
            foreach ($files as $key => $file) {
                if ($file->isValid() && !$file->hasMoved()) {
                    ...
                    if (isset($data['nisn'])) {
                        $sub = 'siswa/' . $data['nisn'];
                    } elseif (strpos('auth_referensi', 'wb_') !== false) {
                        $sub = str_replace('wb_', '', 'auth_referensi');
                    }
