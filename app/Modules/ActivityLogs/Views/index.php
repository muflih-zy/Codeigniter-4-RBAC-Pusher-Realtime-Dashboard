...
<div class='container-xl'>
    <div class='card shadow-sm border-0'>
        ...
        <div class='table-responsive p-3'>
            <table id='t-activity-logs' class='table table-vcenter card-table table-striped w-100'>
                <thead>
                    <tr>
                        <th width='5%'>NO</th>
                            <th>USER ID</th>
...
<script>
    ...
    $(document).ready(function() {
        table = $('#t-activity-logs').DataTable({
            processing: true, 
            serverSide: true,
            order: [[1, 'asc']],
            ajax: { 
                url: '<?= base_url('activity-logs/ajaxData') ?>', 
                type: 'POST', 
                data: d => { d.<?= csrf_token() ?> = '<?= csrf_hash() ?>'; } 
            },
            columns: [
                { data: 'no', orderable: false, searchable: false, className: 'text-center' },
                { data: 'user_id', defaultContent: '-', className: 'text-nowrap' },
                { data: 'action', defaultContent: '-', className: 'text-nowrap' },
                { data: 'message', defaultContent: '-', className: 'text-nowrap' },
    ...
    function addData() { $.get('<?= base_url('activity-logs/create') ?>', res => { $('#modal-content-area').html(res); $('#modal-form').modal('show'); }); }
    function editData(id) { $.get('<?= base_url('activity-logs/edit/') ?>' + id, res => { $('#modal-content-area').html(res); $('#modal-form').modal('show'); }); }
    function deleteData(id) { deleteId = id; $('#modal-confirm').modal('show'); }

    $('#btn-confirm-del').on('click', function() { 
        $.post('<?= base_url('activity-logs/delete/') ?>' + deleteId, { '<?= csrf_token() ?>': '<?= csrf_hash() ?>' }, res => {
