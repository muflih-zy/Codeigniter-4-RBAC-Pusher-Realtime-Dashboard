...
use Modules\ActivityLogs\Models\ActivityLogsModel;
...
{
    protected $model;
    protected $moduleUrl = 'activity-logs';
    ...
    public function index() {
        ...
        return view('Modules\ActivityLogs\Views\index', [
            'title'  => 'Kelola ActivityLogs',
            'url'    => $this->moduleUrl,
            'access' => $access
        ]);
    }
    ...
    public function ajaxData() {
        ...
        $builder = $db->table('auth_activity_logs');
        $builder->join('auth_users as user_id_ref', 'user_id_ref.id = auth_activity_logs.user_id', 'left');
        $builder->select("auth_activity_logs.id, auth_activity_logs.user_id, auth_activity_logs.action, auth_activity_logs.message, user_id_ref.username as user_id_label");
        $dt = DataTable::of($builder)->addNumbering('no');

        $dt->edit('user_id', function($row) {
            return $row->user_id_label ?? '-';
        });
    ...
    public function create() {
        ...
        return view('Modules\ActivityLogs\Views\form', ['url' => $this->moduleUrl]); 
    }

    public function edit($id) {
        if ($this->getAccess()['can_update'] != 1) return 'Akses dilarang';
        $data = $this->model->find($id);
        return view('Modules\ActivityLogs\Views\form', ['row' => $data, 'url' => $this->moduleUrl]);
    }
    ...
    public function store() {
        ...
        if ($this->model->save($data)) {
            $this->notify('auth_activity_logs');
            ActivityLogModel::saveLog('INSERT', "Menambah ActivityLogs", 'ti ti-plus', 'success');
    ...
    public function update($id) {
        ...
        if ($this->model->update($id, $data)) {
            $this->notify('auth_activity_logs');
            ActivityLogModel::saveLog('UPDATE', "Update ActivityLogs ID: $id", 'ti ti-edit', 'info');
        ...
        if (!empty($files)) {
            foreach ($files as $key => $file) {
                if ($file->isValid() && !$file->hasMoved()) {
                    ...
                    if (isset($data['nisn'])) {
                        $sub = 'siswa/' . $data['nisn'];
                    } elseif (strpos('auth_activity_logs', 'wb_') !== false) {
                        $sub = str_replace('wb_', '', 'auth_activity_logs');
                    }
